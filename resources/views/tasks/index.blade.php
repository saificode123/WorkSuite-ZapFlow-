    ...
    <script>
        ...
        $('#allTasks-table').on('change', '.change-status', function() {
            ...
            function hasPrivilegedRole() {
                // Roles that can bypass approval and downgrade restrictions
                var privilegedRoles = ['admin', 'administrator', 'manager', 'hr'];

                for (var i = 0; i < roles.length; i++) {
                    if (privilegedRoles.includes(roles[i].name)) {
                        return true;
                    }
                }
                return false;
            ...
            if (id != "" && status != "") {
                if(status == 'completed' && !hasPrivilegedRole() && projectAdmin != loginUser && needApproval == 1){
                    Swal.fire({
                        title: "@lang('messages.sweetAlertTitle')",
                    ...
                    $.easyAjax({
                        ...
                        success: function(response) {
                            ...
                            // Update the previous status after successful change
                            selectElement.data('previous-status', status);
                        },
