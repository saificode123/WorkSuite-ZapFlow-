...
@php
$addPermission = user()->permission('add_airline');
@endphp
...
@push('scripts')
    ...
    <script>
        const showTable = () => window.LaravelDataTables["airlines-table"].draw(true);
        $('body').on('click', '.delete-table-row', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.easyAjax({
                        type: 'POST', url: "{{ route('airlines.destroy', ':id') }}".replace(':id', id),
