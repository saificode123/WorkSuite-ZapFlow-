...
@php
$addPermission = user()->permission('add_customer_type');
@endphp
        ...
        <div class="d-block d-lg-flex d-md-flex justify-content-between align-items-center mb-2 ct-page-header">
            ...
            <div id="table-actions" class="flex-grow-1 align-items-center mb-2 mb-lg-0 mb-md-0">
                @if (in_array($addPermission, ['all', 'added']))
                    <x-forms.link-primary :link="route('customer-types.create')" class="mr-3 float-left openRightModal" icon="plus">
                        @lang('app.add') @lang('app.customerType')
...
@push('scripts')
    ...
    <script>
        $('#customer-types-table').on('preXhr.dt', function(e, settings, data) {
            data['searchText'] = $('#search-text-field').val();
        });
        const showTable = () => window.LaravelDataTables["customer-types-table"].draw(true);
        $('body').on('click', '.delete-table-row', function() {
            var id = $(this).data('customer-type-id');
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.easyAjax({
                        type: 'POST', url: "{{ route('customer-types.destroy', ':id') }}".replace(':id', id),
