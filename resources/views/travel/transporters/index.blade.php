...
@php
$addPermission = user()->permission('add_transporter');
@endphp
        ...
        <div class="add-client bg-white rounded shadow-sm">
            <div class="d-block d-lg-flex d-md-flex justify-content-between align-items-center border-bottom-grey p-20">
                <h4 class="mb-0 f-21 font-weight-normal">
                    <i class="fa fa-truck mr-2 text-primary"></i>Transporters
                </h4>
                <div id="table-actions" class="mt-2 mt-lg-0 mt-md-0">
                    @if (in_array($addPermission, ['all', 'added']))
                        <x-forms.link-primary :link="route('transporters.create')" class="mr-3 float-left openRightModal" icon="plus">
                            @lang('app.add') @lang('app.transporter')
...
@push('scripts')
    ...
    <script>
        const showTable = () => window.LaravelDataTables["transporters-table"].draw(true);
        $('body').on('click', '.delete-table-row', function() {
            var id = $(this).data('transporter-id');
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.easyAjax({
                        type: 'POST', url: "{{ route('transporters.destroy', ':id') }}".replace(':id', id),
