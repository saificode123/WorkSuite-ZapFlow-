...
<div class="content-wrapper">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="mb-0">@lang('modules.travelPayments.makePayments')</h4>
        @if (in_array(user()->permission('add_travel_payment'), ['all', 'added']))
            <x-forms.link-primary :link="route('travel-payments.make.create')" class="openRightModal" icon="plus">
                @lang('app.makePayment')
            </x-forms.link-primary>
        @endif
    </div>

    <div class="d-flex flex-column w-tables rounded mt-3 bg-white w-100 table-responsive">
        <table class="table table-hover border-0 w-100" id="make-payments-table">
            <thead>
                <tr>
                    <th>@lang('app.id')</th>
                    <th>@lang('app.date')</th>
                    <th>@lang('app.paidTo')</th>
                    <th>@lang('app.reference')</th>
                    <th>@lang('app.creditAccount')</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $payment->creditAccount?->name ?? '—' }}</td>
