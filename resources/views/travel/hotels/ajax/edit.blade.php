        ...
        <x-form id="save-data-form" method="PUT">
            <div class="add-client bg-white rounded shadow-sm ct-card">
                ...
                <div class="row p-20">
                    ...
                    <div class="col-md-6">
                        <x-forms.number fieldId="stars" :fieldLabel="__('modules.hotel.starRating')" fieldName="stars" min="1" max="5" fieldValue="{{ $hotel->stars }}"></x-forms.number>
                    ...
                    <div class="col-md-6 mt-3">
                        <x-forms.email fieldId="email" :fieldLabel="__('app.email')" :fieldPlaceholder="__('app.email')" fieldName="email" fieldValue="{{ $hotel->email }}"></x-forms.email>
                    </div>
                    <div class="col-md-6 mt-3">
                        <x-forms.text fieldId="phone" :fieldLabel="__('app.phone')" fieldName="phone" fieldValue="{{ $hotel->phone }}"></x-forms.text>
                    </div>
                </div>
                <x-form-actions>
...
<script>
    ...
    // "stars" field, seeded from the hotel's current rating on load.
    // Read-only — never writes to the field or validates its bounds.
    (function() {
        var $stars = $('#stars');
        ...
        updateStarsPreview(); // reflects $hotel->stars immediately
    })();
</script>