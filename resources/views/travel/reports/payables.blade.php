...
<div class="content-wrapper rpt-box">
    <div class="rpt-mast">
        <div>
            <h1><i class="fa fa-money-bill-wave mr-2 text-danger"></i>Payables Report</h1>
            <p class="text-muted mb-0">Supplier & Vendor Accounts Payable Outflows (Hotels, Transport, Airlines, Visa)</p>
        </div>
        <div>
    ...
    <div class="rpt-filter">
        <form method="GET" action="{{ route('travel-reports.payables') }}" class="row g-2 align-items-end">
            <div class="col-md-3">
            ...
            <div class="col-md-3">
                ...
                <a href="{{ route('travel-reports.payables') }}" class="btn btn-sm btn-light border ml-1"><i class="fa fa-times mr-1"></i>Reset</a>
            </div>
        </form>
    </div>
        ...
        <table class="rpt-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="font-weight-bold">{{ $p->partyUser?->name ?? 'Vendor' }}</td>
                    ...
                    <td class="text-right font-mono font-weight-bold text-danger">{{ number_format($p->amount_base_currency, 2) }}</td>
                </tr>
                @empty
                <tr><td colspan="9" class="text-center text-muted py-4">No payable records found for this period.</td></tr>
                @endforelse
            </tbody>
            ...
            <tfoot>
                <tr style="background:#fafbfe; font-weight:700;">
                    ...
                    <td class="text-right font-mono text-danger">{{ number_format($payments->sum('amount'), 2) }}</td>
                    <td class="text-right font-mono text-danger">{{ number_format($payments->sum('amount_base_currency'), 2) }}</td>
                </tr>
            </tfoot>
            @endif
        </table>
