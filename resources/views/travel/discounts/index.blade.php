...
@php
$addPermission = user()->permission('add_discount');
@endphp
        ...
        <div class="d-block d-lg-flex d-md-flex justify-content-between align-items-center mb-2 ct-page-header">
            ...
            <div id="table-actions" class="flex-grow-1 align-items-center mb-2 mb-lg-0 mb-md-0">
                @if (in_array($addPermission, ['all', 'added']))
                    <x-forms.link-primary :link="route('discounts.create')" class="mr-3 float-left openRightModal" icon="plus">
                        @lang('app.add') @lang('app.discount')
...
@push('scripts')
    ...
    <script>
        const showTable = () => window.LaravelDataTables["discounts-table"].draw(true);
        $('body').on('click', '.delete-table-row', function() {
            var id = $(this).data('discount-id');
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.easyAjax({
                        type: 'POST', url: "{{ route('discounts.destroy', ':id') }}".replace(':id', id),
