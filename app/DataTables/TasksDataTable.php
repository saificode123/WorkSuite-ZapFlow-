...
//        }
        ...
        // Find this section in your TasksDataTable.php (around line 344)
// Replace the entire board_column editColumn with this updated version:
        ...
        $datatables->editColumn('board_column', function ($row) use ($taskBoardColumns, $userId, $clientIds) {
            ...
            if ($this->changeStatusPermission == 'all'
                ...
                if (($row->board_column_id == $taskBoardColumn->id) && (in_array('admin', user_roles()) || (in_array('employee', user_roles()) && $row->project_admin == user()->id))) {
                    ...
                    // ✅ UPDATED: Added data-previous-status and data-previous-priority attributes
                ...
                data-task-id="' . $row->id . '"
                data-previous-status="' . $row->boardColumn->slug . '"
                data-previous-priority="' . $row->boardColumn->priority . '">';
                    ...
                    foreach ($taskBoardColumns as $item) {
                        if ($item->id != $taskBoardColumn->id) {
                            ...
                            // ✅ UPDATED: Added data-priority attribute to each option
                            $status .= ' data-priority="' . $item->priority . '" data-content="<i class=\'fa fa-circle mr-2\' style=\'color: ' . $item->label_color . '\'></i> ' . $item->column_name . '" value="' . $item->slug . '">' . $item->column_name . '</option>';
                        }
                    }
