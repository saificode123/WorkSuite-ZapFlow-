...
class CustomerTypeDataTable extends BaseDataTable
{
    ...
    {
        parent::__construct();
        $this->viewPermission = user()->permission('view_customer_type');
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                if (in_array($this->editPermission, ['all', 'added'])) {
                    $action .= '<a class="dropdown-item openRightModal" href="' . route('customer-types.edit', [$row->id]) . '">
                ...
                if (in_array($this->deletePermission, ['all', 'added'])) {
                    $action .= '<a class="dropdown-item delete-table-row" href="javascript:;" data-customer-type-id="' . $row->id . '">
    ...
    {
        return $this->setBuilder('customer-types-table', 2)
            ->parameters([
                'initComplete' => 'function () {
                    window.LaravelDataTables["customer-types-table"].buttons().container()
