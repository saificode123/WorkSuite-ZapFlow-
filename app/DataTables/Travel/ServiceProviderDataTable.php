...
class ServiceProviderDataTable extends BaseDataTable
{
    ...
    {
        parent::__construct();
        $this->viewPermission = user()->permission('view_service_provider');
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                if (in_array($this->editPermission, ['all', 'added'])) {
                    $action .= '<a class="dropdown-item openRightModal" href="' . route('service-providers.edit', [$row->id]) . '">
                ...
                if (in_array($this->deletePermission, ['all', 'added'])) {
                    $action .= '<a class="dropdown-item delete-table-row" href="javascript:;" data-service-provider-id="' . $row->id . '">
    ...
    {
        return $this->setBuilder('service-providers-table', 2)
            ->parameters([
                'initComplete' => 'function () {
                    window.LaravelDataTables["service-providers-table"].buttons().container()
