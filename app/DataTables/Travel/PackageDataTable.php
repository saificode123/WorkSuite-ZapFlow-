...
class PackageDataTable extends BaseDataTable
{
    ...
    {
        parent::__construct();
        $this->viewPermission = user()->permission('view_package');
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                if (in_array($this->editPermission, ['all', 'added'])) {
                    $action .= '<a class="dropdown-item openRightModal" href="' . route('packages.edit', [$row->id]) . '">
                ...
                if (in_array($this->deletePermission, ['all', 'added'])) {
                    $action .= '<a class="dropdown-item delete-table-row" href="javascript:;" data-package-id="' . $row->id . '">
    ...
    {
        return $this->setBuilder('packages-table', 2)
            ->parameters([
                'initComplete' => 'function () {
                    window.LaravelDataTables["packages-table"].buttons().container()
    ...
    protected function getColumns()
    {
        return [
            ...
            __('modules.package.durationDays') => ['data' => 'duration_days', 'name' => 'duration_days', 'title' => __('modules.package.durationDays')],
