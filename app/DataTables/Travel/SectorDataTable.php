...
class SectorDataTable extends BaseDataTable
{
    ...
    {
        parent::__construct();
        $this->viewPermission = user()->permission('view_sector');
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                if (in_array($this->editPermission, ['all', 'added'])) {
                    $action .= '<a class="dropdown-item openRightModal" href="' . route('sectors.edit', [$row->id]) . '">
                ...
                if (in_array($this->deletePermission, ['all', 'added'])) {
                    $action .= '<a class="dropdown-item delete-table-row" href="javascript:;" data-sector-id="' . $row->id . '">
    ...
    {
        return $this->setBuilder('sectors-table', 2)
            ->parameters([
                'initComplete' => 'function () {
                    window.LaravelDataTables["sectors-table"].buttons().container()
