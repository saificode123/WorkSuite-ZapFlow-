...
class VoucherDataTable extends BaseDataTable
{
    ...
    {
        parent::__construct();
        $this->viewPermission = user()->permission('view_voucher');
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                $action .= '<a href="' . route('vouchers.show', [$row->id]) . '" class="dropdown-item"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';

                if (in_array($this->editPermission, ['all', 'added'])) {
                    $action .= '<a class="dropdown-item openRightModal" href="' . route('vouchers.edit', [$row->id]) . '">
    ...
    {
        return $this->setBuilder('vouchers-table', 2)
            ->parameters([
                'initComplete' => 'function () {
                    window.LaravelDataTables["vouchers-table"].buttons().container()
    ...
    protected function getColumns()
    {
        return [
            ...
            __('modules.voucher.voucherNumber') => ['data' => 'voucher_number', 'name' => 'voucher_number', 'title' => __('modules.voucher.voucherNumber')],
            __('app.date') => ['data' => 'date', 'name' => 'date', 'title' => __('app.date')],
            __('modules.voucher.totalAmount') => ['data' => 'total_amount', 'name' => 'total_amount', 'title' => __('modules.voucher.totalAmount')],
