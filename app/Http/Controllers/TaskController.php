...
class TaskController extends AccountBaseController
{
    ...
    public function changeStatus(Request $request)
    {
        ...
        $task = Task::withTrashed()->with('project', 'users', 'boardColumn')->findOrFail($taskId);
        ...
        $newTaskBoardColumn = TaskboardColumn::where('slug', $status)->first();

        // Get user roles
        $userRoles = user_roles();

        // Define roles that CAN downgrade tasks (have full status control)
        $privilegedRoles = ['admin', 'administrator', 'manager', 'hr'];

        // Check if user has any privileged role
        $hasPrivilegedRole = !empty(array_intersect($privilegedRoles, $userRoles));

        // If user doesn't have privileged role, prevent downgrade
        if (!$hasPrivilegedRole) {
            $currentColumn = $task->boardColumn;

            // Check if the new status has lower priority (downgrade)
            if ($newTaskBoardColumn->priority < $currentColumn->priority) {
                return Reply::error(__('messages.cannotDowngradeTaskStatus'));
            }
        }

        $task->board_column_id = $newTaskBoardColumn->id;

        if ($task->status === 'completed' && $status !== 'completed') {
            $task->approval_send = 0; // Reset approval_send to 0
        }

        if ($newTaskBoardColumn->slug == 'completed') {
    ...
    public function store(StoreTask $request)
    {
        ...
        $task->save();
        // ── BugHerd mirror (simple inline call) ───────────────────────────────────────
        try {
            // Only attempt if project is linked to BugHerd
            $bugherdProjectId = (int) ($task->project->bugherd_project_id ?? 0);
            if ($bugherdProjectId) {

                /** @var \App\Services\Bugherd $api */
                $api = app(Bugherd::class);

                // Build description: heading + stripped HTML description
                $heading      = trim((string) $task->heading);
                $html         = (string) ($request->input('description') ?? $task->description ?? '');
                $plain        = trim(strip_tags($html));
                $description  = trim($heading . (strlen($plain) ? "\n\n{$plain}" : ''));

                // Labels -> tag_names (optional; comment out if you don’t use labels)
                $tagNames = [];
                if ($request->filled('task_labels')) {
                    // If you store labels on pivot, load their names here.
                    // Adjust the relation / query to your app:
                    $tagNames = $task->labels()->pluck('label_name')->filter()->values()->all();
                }
                ...
                // Due date (optional)
                if ($task->due_date) {
                    // BugHerd expects ISO-8601 in UTC
                    $payload['due_at'] = $task->due_date->clone()->utc()->format('Y-m-d\TH:i:s\Z');
                }

                // Column mapping (optional) – map your local board_column_id to BugHerd column_id
                if ($task->board_column_id) {
                    $map = [
                        // local_board_column_id  => bugherd_column_id
                        // TODO: replace with your real IDs (from BugHerd /projects/:id/columns)
                        // e.g. To Do => 2068559, Doing => 2068560, Completed => 2068561
                        // 1 => 2068559,
                        // 2 => 2068560,
                        // 3 => 2068561,
                    ];
                    if (isset($map[$task->board_column_id])) {
                        $payload['column_id'] = (int) $map[$task->board_column_id];
                    }
                }

                // Priority mapping (optional)
                // $payload['priority_id'] = match($task->priority) {
                //     'high'   => 3,
                //     'medium' => 2,
                //     'low'    => 1,
                //     default  => 0, // not set
                ...
                // Create the task in BugHerd
                $resp = $api->createTask($bugherdProjectId, $payload);

                // If you added a column `bugherd_task_id` on tasks, store it:
                if ($bugherdId = data_get($resp, 'task.id')) {
                    ...
                    Log::warning('BugHerd createTask returned no id', ['task_id' => $task->id, 'resp' => $resp]);
                }
            }
        } catch (\Throwable $e) {
            // Don’t break your normal flow if BugHerd fails
            Log::error('BugHerd createTask failed', [
                'task_id' => $task->id,
