...
use App\Services\XeroService; // <--- IMPORTANT: XeroService is imported here
...
class InvoiceController extends AccountBaseController
{
    ...
    public function create()
    {
        ...
        $invoice = new Invoice();
        $invoice->auto_sync_xero = true; // or 1
...
// TEST: Check if this line runs
        \Log::info('INVOICE SAVED - Testing Xero sync', [
            'invoice_id' => $invoice->id,
            'auto_sync_xero' => $invoice->auto_sync_xero
        ]);

// XERO AUTO-SYNC INTEGRATION START
        // XERO AUTO-SYNC INTEGRATION START
        $syncStatusMessage = '';
        try {
            ...
            \Log::info('Xero Auto-sync check started', [
                'invoice_id' => $invoice->id,
                'company_id' => $companyId,
                'auto_sync_xero' => $invoice->auto_sync_xero,
                'is_connected' => $xeroService->isConnected($companyId)
            ]);

            if ($xeroService->isConnected($companyId) && $invoice->auto_sync_xero) {
                \Log::info('Attempting to create invoice in Xero', ['invoice_id' => $invoice->id]);

                $result = $xeroService->createInvoice($invoice);

                \Log::info('Xero sync result', [
                    'invoice_id' => $invoice->id,
                ...
                if (!$result['success']) {
                    $syncStatusMessage = ' (Xero Sync Failed: Check DB for error)';
                } else {
                    $syncStatusMessage = ' (Xero Sync Success)';
                }
            } else {
                \Log::warning('Xero sync skipped', [
                    'invoice_id' => $invoice->id,
                    'is_connected' => $xeroService->isConnected($companyId),
                    'auto_sync_enabled' => $invoice->auto_sync_xero
                ]);
            }
        } catch (\Exception $e) {
            $syncStatusMessage = ' (Xero Sync Error: Connection Failure)';
            \Log::error('Xero Auto-sync exception for invoice ' . $invoice->id, [
                'message' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
        }
// XERO AUTO-SYNC INTEGRATION END
        ...
        $message = __('messages.recordSaved') . $syncStatusMessage;

        if ($invoice->send_status == 1) {
            $message = __('messages.invoiceSentSuccessfully') . $syncStatusMessage;
        }


        return Reply::successWithData($message, ['redirectUrl' => $redirectUrl, 'invoiceID' => $invoice->id]);
    }

    // The update method is also fixed to correctly handle the error storage
    public function update(UpdateInvoice $request, $id)
    {
        ...
        $invoice->payment_status = $request->payment_status == null ? '0' : $request->payment_status;
        $invoice->invoice_payment_id = $request->invoice_payment_id;
        $invoice->auto_sync_xero = true; // ADD THIS LINE BEFORE SAVE
        ...
        try {
            ...
            if ($xeroService->isConnected($companyId) && $invoice->auto_sync_xero) {
                // If invoice already has a Xero ID, update it; otherwise, try to create it.
                if ($invoice->xero_invoice_id) {
                    $result = $xeroService->updateInvoice($invoice);
                } else {
                    $result = $xeroService->createInvoice($invoice);
                }

                if (!$result['success']) {
                    // Sync failed, message stored in database column by service
                    $syncStatusMessage = ' (Xero Sync Failed: Check DB for error)';
                } else {
                    $syncStatusMessage = ' (Xero Sync Success)';
                }
        ...
        $redirectUrl = route('invoices.index');
        $message = __('messages.updateSuccess') . $syncStatusMessage;
    ...
    public function destroy($id)
    {
        ...
        abort_403(!($this->deletePermission == 'all'
            ...
            || ($this->deletePermission == 'both' && ($invoice->added_by == user()->id || $invoice->client_id == user()->id))
        ));
        ...
        // Delete invoice item images (correct relationship)
        $invoiceItems = InvoiceItems::where('invoice_id', $invoice->id)->get();
        foreach ($invoiceItems as $item) {
            $images = InvoiceItemImage::where('invoice_item_id', $item->id)->get();
            foreach ($images as $image) {
                Files::deleteFile($image->hashname, InvoiceItemImage::FILE_PATH . '/' . $image->id . '/');
                $image->delete();
            }
        }
        ...
        // If connected to Xero and has Xero ID, log it
        if ($invoice->xero_invoice_id) {
            try {
                $xeroService = new \App\Services\XeroService();
                $companyId = company()->id;

                if ($xeroService->isConnected($companyId)) {
                    \Log::info('Invoice deleted locally, Xero invoice ID: ' . $invoice->xero_invoice_id);
                }
            } catch (\Exception $e) {
                \Log::error('Error handling Xero invoice deletion: ' . $e->getMessage());
            }
        }
    ...
    /**
     * Create DOM PDF object for download
     */
public function domPdfObjectForDownload($id)
{
    $this->invoice = Invoice::with('items', 'client', 'client.clientDetails', 'payment')->findOrFail($id);
    $this->paidAmount = $this->invoice->getPaidAmount();
    $this->creditNote = 0;
    ...
    // IMPORTANT: Return array with both pdf and filename
