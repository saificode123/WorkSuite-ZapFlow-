    ...
    public function permissions()
    {
        ...
        if ($this->role->name == 'client') {
            ...
            $this->modulesData = Module::with('permissions')->withCount('customPermissions')
                ->whereIn('module_name', $clientModules)
                ->where('module_name', '<>', 'messages')
                ->where('module_name', '<>', 'products')
                ->where('module_name', '<>', 'orders')// Add this line
                ->get();
        }
        else {
            $this->modulesData = Module::with('permissions')
                ->where('module_name', '<>', 'messages')
                ->where('module_name', '<>', 'products')
                ->where('module_name', '<>', 'orders')// Add this line
                ->withCount('customPermissions')
                ->get();
        }
